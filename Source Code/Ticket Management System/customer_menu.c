...
void viewFAQ()
{
	...
	wprintf(L"A: Try moving the mouse or hitting any key on the keyboard.\nIf that fails, locate the power button on the screen and press it/ncheck the connectivity of the screen monitor to the computer, is and check if the screen is on.\n");
	wprintf(L"\n");
	...
	wprintf(L"A: first of all check the electric supply, and if the switch behind the computer is on, after that try to disconnect the cable cord from the computer, and keep the power button pressed 10 seconds,/n then reinsert the power cord and try to turn on the computer.\n");
	wprintf(L"\n");
	wprintf(L"Q: My network doesn't work(there's an X symbol over the internet symbol)\n");
	wprintf(L"A: Try to check if there is light comming up from the ethernet cable (it's on the lower side behind the computer) check connectivity of said cable, right click that icon, make sure the internet isn't dissabled, if using netstick try to reinset the netstick.\n");
	wprintf(L"\n");
	wprintf(L"Q: My computer works slowly!\n");
	wprintf(L"A: Try to close programs to reduce work stress on the computer, and if you didn't restarted the computer in a while do so, if that doesn't help then delete temps on the computer, you can find them by seraching %temp% in the search bar in the start menu, check if the C: drive is full. \n");
	wprintf(L"\n");
	wprintf(L"Q: A program doesn't start no matter how many times I try to open it\n");
	wprintf(L"A: Try pressing CTRL + ALT + DEL or ctrl + shift + esc to open task manager, search for the program's name press right click and then end process, now try to open the program again.\n");
	wprintf(L"\n");
	system("PAUSE");
}
